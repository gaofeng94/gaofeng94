...
%% 渐变介质层
%计算一个周期的折射率变化
spacing1=0.1*t1  %inplane distance
spacing2=0.1*t1   %cross section distance
H=(((t1+spacing2)^2-((t1+spacing1)/2)^2)^0.5)  %垂直方向球间距
a1=H-(t1/2);  %第一边界
a1=round(a1)
a2=H/2  %第二边界
a2=round(a2)
x1=0:grid:a1
x2=a1+1:grid:a2
f1=(((t1/2)^2-x1.^2).^0.5)/(t1/2+spacing1/2)
f2=(((t1/2)^2-x2.^2).^0.5)+(((t1/2)^2-(t1/2-(x2-a1)).^2).^0.5)
f2=f2/((t1+spacing1)/2)
f=[f1 f2]
